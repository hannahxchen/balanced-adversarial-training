...
class MinWordEmbeddingDistance(Constraint):
    """A constraint on word substitutions for finding words that are far from the target word.
    THis places a minimum distance between the embedding of the word being deleted and the word being
    inserted.
    Args:
        ...
        max_cos_sim (:obj:`float`, optional): The maximum cosine similarity between word embeddings.
        min_mse_dist (:obj:`float`, optional): The minimum euclidean distance between word embeddings.
    ...
    def __init__(
        ...
        if bool(max_cos_sim) == bool(min_mse_dist):
            raise ValueError("You must choose either `max_cos_sim` or `min_mse_dist`.")
        self.max_cos_sim = max_cos_sim
        self.min_mse_dist = min_mse_dist
    ...
    def _check_constraint(self, transformed_text, reference_text):
        ...
        for i in indices:
            ...
            if self.max_cos_sim:
                ...
                if cos_sim > self.max_cos_sim:
                    return False
            # Check MSE distance.
            if self.min_mse_dist:
                mse_dist = self.get_mse_dist(ref_id, transformed_id)
                if mse_dist < self.min_mse_dist:
                    return False
