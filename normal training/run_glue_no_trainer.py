...
def main():
    ...
    if args.task_name is not None:
        ...
        # if args.task_name == "wiki-toxic":
        #     raw_datasets["train"].features["label"] = datasets.ClassLabel(names=["toxic", "non-toxic"])
        #     raw_datasets["validation"].features["label"] = datasets.ClassLabel(names=["toxic", "non-toxic"])
    ...
    eval_dataloader = DataLoader(
        eval_dataset, collate_fn=data_collator, batch_size=args.per_device_eval_batch_size * args.n_gpu
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            if args.task_name in ["snli", "mnli", "qqp", "mrpc"]:
                metric.add_batch(
        ...
        if args.task_name in ["mnli", "qqp", "mrpc", "snli"]:
            eval_metric = metric.compute()
        else:
            eval_metric = acc_metric.compute()
            eval_metric2 = f1_metric.compute()
            eval_metric["f1"] = eval_metric2["f1"]
        ...
        with open(os.path.join(save_dir, f"eval_results.json"), "w") as f:
